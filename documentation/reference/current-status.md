...
> **版本 v3.0.0-alpha (Phase G)** | 更新 2026-05-26  
> 文档位置：`documentation/reference/current-status.md`  
> 代码仓库: `ttmens/stock-copilot` | 线上: https://ttmens.github.io/stock-copilot/

## Phase G 要点（3.0.0-alpha）

| ID | 能力 | 模块 | 状态 |
|----|------|------|------|
| G1 | 日更知识库 | `src/intelligence/` | ✅ |
| G2–G3 | 外盘/期货 | `overnight.py`, `futures.py` | ✅ |
| G4 | 推荐池 3×3 | `src/recommendation/` | ✅ |
| G5 | 竞价监测 | `src/monitoring/auction.py` | ✅ |
| G6 | 盘中预警 | `intraday.py`, `alerts.py` | ✅ |
| G7 | 复盘/深度 | `src/review/`, `deep_research.py` | ✅ |
| G8 | 仓位 CRUD | `src/portfolio/` | ✅ |
| G-UI | 双轨 Live Cockpit | `src/site/app/*.html`, `live.js` | ✅ |

API：`/api/digest/today`, `/api/recommendations/today`, `/api/auction/latest`, `/api/alerts`, `/api/positions`, `/api/review/today`, `/api/market/session`

静态 JSON：`data/digest.json`, `data/recommendation.json`, `data/review.json`

## 实现 vs 文档 Gap（2026-05-26）

| 领域 | 实现 | 文档/计划 |
|------|------|-----------|
| 测试数量 | ~97 pytest | 旧文档写 36 |
| 新闻 | AkShare `stock_news_em` ✅ | 旧 data-sources 写禁用 — 已修正 |
| 多空辩论 | `debate.py` 已接入 | Phase C 曾标 Out of Scope；现属 D-mirofish |
| API 端点 | 20+（见 api-spec.md） | 旧 05-API 仅 5 个 — 已重写 |
| Phase F 停滞/相关/多周期 | ❌ Pending | workflow 标 partial |
| Thesis transition API/UI | Partial | Pending |
| routes 重复注册 | ✅ 已修复 (2026-05-30) | — |

完整 Phase 矩阵见 [VERSION.md](../VERSION.md)。
