# Stock Copilot Web UI 产品说明 (Phase G)

> **视觉 SSOT**: [design-system/tokens.md](../design-system/tokens.md)  
> **Phase G UI SSOT**: [ui-phase-g.md](ui-phase-g.md)  
> 实现: `src/site/generator.py` + `src/site/app/` + `docs/assets/theme.css`

## Phase G 双轨 UI（单页 Cockpit + Journey）

- **主入口**：`app/cockpit.html` — 今日 / 自选 / 我的 三 Tab
- **Journey**：Now Hero → Timeline → Focus 面板 / Peek 摘要
- **渲染 SSOT**：`ui-render.js` 与 generator TPL_HOME 卡片结构一致
- **实时轨**：FastAPI + `cockpit.js` 轮询
- **双端**：375px bottom-nav + Hero 吸顶；1280px 盘前双列 + 盯盘双栏

## 视觉回归（验收）

- 自选卡含 decision-card、metrics-row、signal-badge（同 index）
- journey-hero + journey-timeline active 态
- `ui_acceptance.py --full` ≥85 分

## Phase C 交互（保留）

- 自选 Tab：`latest.json` 首屏 + API 动态合并
- 筛选/排序（cockpit.js watchlist 段）
- 单页详情 `app/stock.html?code=`（含深度挖掘折叠区）
...
1. 维护 SSOT：[design-system/tokens.md](../design-system/tokens.md)
